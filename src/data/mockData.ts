import type { DataDomain, QueryResult, SessionGroup } from '../types';
...
export const DEFAULT_QUERY_MAP: Array<{ keywords: string[]; resultId: string }> = [
  ...
  { keywords: ['emea', 'regional', 'london', 'berlin', 'europe'], resultId: 'emea-headcount' },
  { keywords: ['service', 'margin', 'delivery', 'consulting', 'practice', 'utilisation', 'emea'], resultId: 'service-margins' },
];

export function findResult(query: string): QueryResult {
...
export const SESSION_HISTORY: SessionGroup[] = [
  {
    label: 'Today',
    sessions: [
      { id: 's1', query: "What's driving the Q3 headcount cost increase?", timestamp: '08:30', resultId: 'q3-headcount' },
      { id: 's2a', query: "Show me the EMEA regional headcount breakdown", timestamp: '08:45', resultId: 'emea-headcount' },
      { id: 's2', query: 'Show me APAC tech spend vs budget', timestamp: '09:15', resultId: 'apac-tech-spend' },
    ],
  },
  {
    label: 'Yesterday',
    sessions: [
      { id: 's3', query: 'Where are service margins being compressed?', timestamp: 'Jun 12', resultId: 'service-margins' },
    ],
  },
  {
    label: 'This Week',
    sessions: [
      { id: 's4', query: 'Compare revenue performance across all verticals', timestamp: 'Jun 11', resultId: 'q3-headcount' },
      { id: 's5', query: 'What caused the Q2 cloud infrastructure spike?', timestamp: 'Jun 10', resultId: 'apac-tech-spend' },
      { id: 's6', query: 'Summarize workforce efficiency metrics', timestamp: 'Jun 09', resultId: 'q3-headcount' },
    ],
  },
];
